...
func (cmd *Set) UnmarshalBinary(data []byte) error {
	...
	cmd.Properties1.ParameterCount = (payload[i] & 0x1F)

	i += 1

	for i < int(cmd.Properties1.ParameterCount) {
...
func (cmd *Set) MarshalBinary() (payload []byte, err error) {
...

	{
		...
		val |= (cmd.Properties1.ParameterCount) & byte(0x1F)
	...
	for _, vg := range cmd.Vg1 {
		...
		payload = append(payload, vg.ParameterId)
